...
class OllamaTriage:
    ...
    def __init__(
        ...
        self._base_url = (base_url or os.environ.get("OLLAMA_BASE_URL") or _DEFAULT_BASE_URL).rstrip("/")
    ...
    def triage(self, text: str, location: str) -> TriageResult:
        ...
        try:
            ...
            raise RetryableTriageError(f"Ollama unreachable: {type(exc).__name__}") from exc
            ...
            raise RetryableTriageError(f"Ollama request failed: {type(exc).__name__}") from exc

        try:
            body = response.json()
        except ValueError as exc:
            raise TriageValidationError(f"Ollama response was not valid JSON: {exc}") from exc
