...
def create_complaint(
    data: ComplaintCreate, session: Session = Depends(get_session)
) -> ComplaintOut:
...
def get_complaint(
    complaint_id: UUID, session: Session = Depends(get_session)
) -> ComplaintOut:
...
def list_complaints(
    ...
    return ComplaintListOut(
        items=[ComplaintOut.model_validate(item) for item in items], total=total
    )
...
def update_complaint_status(
    ...
    complaint = complaints_service.update_complaint_status(
        session, complaint_id, body.status
    )
    return ComplaintOut.model_validate(complaint)
